 ...
 * @transaction
 */
function confirmMortgage(confirmMortgage){
    var factory = getFactory();
    var namespace = 'org.acme.biznet';

    var validateMortgageTransaction = factory.newTransaction(namespace, 'ValidateAndCalculateMortgage', guid().toString()); //!!!!!should this be newResource
    ...
    var mortgageAmount = validateAndCalculateMortgage(validateMortgageTransaction); //!!!!!!not sure if this async need to use await?
    ...
    mortgageAmount.borrower = confirmMortgage.mortgageApplier;
    ...
    .then(function (mortgageAgreementRegistry){
        ...
        var saleAgreement = factory.newTransaction(namespace, 'ConfirmPropertySale', guid().toString());
        saleAgreement.buyer = confirmMortgage.mortgageApplier;
        saleAgreement.listing = confirmMortgage.listing;
        if(!confirmPropertySale(saleAgreement)){
